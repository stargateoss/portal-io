 ...
 */
package dev.stargateoss.portal.io.file.comparator;

import java.io.File;
import java.io.Serializable;
import java.util.Comparator;

import dev.stargateoss.portal.io.file.ApacheFileUtils;

/**
 * Compare the <b>length/size</b> of two files for order (see
 * {@link File#length()} and {@link ApacheFileUtils#sizeOfDirectory(File)}).
 * <p>
...
public class SizeFileComparator extends AbstractFileComparator implements Serializable {
     ...
     * using {@link ApacheFileUtils#sizeOfDirectory(File)}
     */
     ...
     * directories is calculated using  {@link ApacheFileUtils#sizeOfDirectory(File)}.
     *
     ...
     *  as size zero (see {@link ApacheFileUtils#sizeOfDirectory(File)}).
     */
    ...
    public int compare(final File file1, final File file2) {
        ...
        if (file1.isDirectory()) {
            size1 = sumDirectoryContents && file1.exists() ? ApacheFileUtils.sizeOfDirectory(file1) : 0;
        } else {
            size1 = file1.length();
        }
        long size2 = 0;
        if (file2.isDirectory()) {
            size2 = sumDirectoryContents && file2.exists() ? ApacheFileUtils.sizeOfDirectory(file2) : 0;
        } else {
            size2 = file2.length();
        }
